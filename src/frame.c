...
struct frame *get_frame_at(size_t index) {
    return (struct frame *) ((unsigned int) FRAMES_START_VADDR + index * sizeof(struct frame));
}

static void init_ram_frames() {
    memset(g_frames_context.frames_meta_start_vaddr, 0, g_frames_context.frames_count * sizeof(struct frame));
...
static void setup_frames_addrs(const memory_map_t *map) {
    ...
    g_frames_context.frames_meta_start_vaddr = (struct frame *) (FRAMES_START_VADDR);
...
//        kprintf("frame %p (ref count %d) at %d at %p\n", fcurrent, fcurrent->ref_count, cursor, g_frames_context.frames_meta_start_vaddr);
